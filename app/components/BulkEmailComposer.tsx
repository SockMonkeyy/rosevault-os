...
const CAMPAIGN_STAGES = [
  ...
  },
  {
    ...
    order: 5,
    description: "Final direct follow-up",
  },
  {
    value: "Nurture",
    order: 6,
    description: "Long-term relationship follow-up",
  },
  {
    value: "Completed",
    order: 7,
    description: "Campaign journey completed",
  },
];
...
export default function BulkEmailComposer({
  ...
  const [campaignStage, setCampaignStage] = useState(
    initialCampaign?.campaign_stage ?? "Introduction",
  );
  ...
  useEffect(() => {
    ...
    // ============================================================
    // SYNCHRONIZE CAMPAIGN CONTENT
    // ============================================================

    setCampaignName(initialCampaign?.name ?? "");

    setSelectedTemplateId(initialCampaign?.template_id ?? "");

    setSubject(initialCampaign?.subject ?? "");

    setBody(initialCampaign?.body ?? "");

    // ============================================================
    // SYNCHRONIZE CAMPAIGN STAGE
    // ============================================================

    setCampaignStage(initialCampaign?.campaign_stage ?? "Introduction");

    // Reset preview when switching campaigns
    setPreviewContactId("");
  }, [initialCampaign, initialSelectedContactIds, contacts]);
  ...
  const selectedStage =
    CAMPAIGN_STAGES.find((stage) => stage.value === campaignStage) ??
    CAMPAIGN_STAGES[0];
  ...
  function changeCampaignStage(stageValue: string) {
    setCampaignStage(stageValue);
  }
  ...
  async function handleSendCampaign() {
    if (!campaign?.id) {
      setDraftMessage("Please save the campaign before sending.");
      return;
    }
    ...
    try {
      ...
      if (!result.success) {
        setDraftMessage(result.error ?? "Unable to send campaign.");
      ...
      setCampaign((current) =>
        current
          ? {
              ...
              campaign_stage: sendResult.stage ?? current.campaign_stage,

              stage_order: sendResult.stageOrder ?? current.stage_order,

              last_template_id:
                sendResult.templateId ?? current.last_template_id,

              last_template_name:
                sendResult.templateName ?? current.last_template_name,
            }
          : current,
      );
      ...
      setDraftMessage("Unexpected error sending campaign.");
    } finally {
      setIsSending(false);
    }
  }
        ...
        <div className="space-y-6">
          <section className="rounded-xl border border-[#EDE7DC] bg-white/40 p-6 backdrop-blur-sm transition-colors duration-300 hover:bg-white/50 lg:p-8">
            ...
            <div className="space-y-5">
              ...
              <div>
                <label className={labelClasses}>Campaign Stage</label>

                <select
                  value={campaignStage}
                  onChange={(event) => changeCampaignStage(event.target.value)}
                  className={inputClasses}
                >
                  {CAMPAIGN_STAGES.map((stage) => (
                    <option key={stage.value} value={stage.value}>
                      {stage.value} — {stage.description}
                    </option>
                  ))}
                </select>

                <div className="mt-2 flex items-center gap-2">
                  <span className="inline-flex rounded-full border border-[#D8B66A]/40 bg-[#B7832F]/5 px-2.5 py-1 text-[9px] font-semibold uppercase tracking-[0.12em] text-[#B7832F]">
                    Stage {selectedStage.order}
                  </span>

                  <span className="text-[10px] text-[#8F8578]">
...
function personalizeText(text: string, contact: Contact) {
  const fullName = [contact.first_name, contact.last_name]
    .filter(Boolean)
    .join(" ");
